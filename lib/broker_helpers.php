...
function generate_broker($_rarity = null)
{
    ...
    try {
        $db = getDB();
        $stmt = $db->prepare("SELECT count(1) as similar FROM `IT202-S25-Brokers` WHERE name like :name");
    ...
    try {
        ...
        if ($id) {
            ...
            foreach ($stocks as $stock) {
                $stmt = $db->prepare("INSERT INTO `IT202-S25-BrokerStocks` (broker_id, stock_id, shares) VALUES (:broker_id, :stock_id,1)");
...
function calculate_stats($rarity, $stocks){
    $db = getDB();
    $stmt = $db->prepare("SELECT property, value FROM `IT202-S25-BrokerProperties`
...
function refresh_broker($id){
    ...
    $broker = [];
    $db = getDB();
    $query = "SELECT id, name, rarity, life, attack, defense FROM `IT202-S25-Brokers` WHERE id = :id";
    try {
    ...
    // fetch stocks
    $stocks = [];
    $query = "SELECT stock_id, quantity, price, volume, low, high FROM `IT202-S25-BrokerStocks` bs join `IT202-S25-Stocks` s ON bs.stock_id = s.id  
    WHERE broker_id = :id";
    try {
        $stmt = $db->prepare($query);
        $stmt->execute([":id" => $id]);
    ...
    if($needsUpdate){
        // update db
        $stmt = $db->prepare("UPDATE `IT202-S25-Brokers` SET life = :life, attack = :attack, defense = :defense, power = :power WHERE id = :id");
