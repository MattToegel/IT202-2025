...
$params = [];
$select = "SELECT c.id, `currency_symbol`, `currency_name`, `average`, `date`";
$query = " FROM `IT202-S25-Crypto` c
WHERE 1=1 AND c.id NOT IN (SELECT uc.crypto_id FROM `IT202-S25-UserCrypto` uc WHERE uc.crypto_id = c.id)";
$count_query = "";
// Filtering logic
if (count($_GET) > 0) {
